...
class ScalazAsyncHandler extends AsyncHandler[Task] {
  ...
  override def receiveBatch[T](
    ...
    completableToTask(consumer.batchReceiveAsync).map(
      _.asScala.map(ConsumerMessage.fromJava).toVector
    )
  ...
  override def transform[A, B](f: Task[A])(fn: A => Try[B]): Task[B] =
    f.flatMap { a =>
      fn(a) match {
  ...
  override def withTransaction[E, A](
    ...
    def close[T](txn: TransactionContext, commit: Boolean, result: T): Task[T] =
      (if (commit) txn.commit(using this) else txn.abort(using this)).map(_ =>
        result
      )
    ...
    startTransaction(builder).flatMap { txn =>
      action(txn)
        .flatMap(result =>
          (if (result.isRight) txn.commit(using this)
           else txn.abort(using this)).map(_ => result)
        )
        .onFinish(errorOpt =>
          if (errorOpt.isDefined) txn.abort(using this) else Task.now(())
        )
    }
  }
