...
class DefaultReader[T](reader: org.apache.pulsar.client.api.Reader[T])
  ...
  override def hasMessageAvailableAsync[F[_]: AsyncHandler]: F[Boolean] =
    implicitly[AsyncHandler[F]].hasMessageAvailable(reader)

  override lazy val topic: Topic = Topic(reader.getTopic)

  override def next: ConsumerMessage[T] =
    ConsumerMessage.fromJava(reader.readNext)

  override def next(duration: Duration): Option[ConsumerMessage[T]] =
    Option(reader.readNext(duration.toSeconds.toInt, TimeUnit.SECONDS))
      .map(ConsumerMessage.fromJava)

  override def nextAsync[F[_]: AsyncHandler]: F[ConsumerMessage[T]] =
    implicitly[AsyncHandler[F]].nextAsync(reader)

  override def isConnected: Boolean = reader.isConnected

  override def close(): Unit = reader.close()
  override def closeAsync[F[_]: AsyncHandler]: F[Unit] =
    implicitly[AsyncHandler[F]].close(reader)
  ...
  override def seekAsync[F[_]: AsyncHandler](timestamp: Long): F[Unit] =
    implicitly[AsyncHandler[F]].seekAsync(reader, timestamp)
  override def seekAsync[F[_]: AsyncHandler](messageId: MessageId): F[Unit] =
    implicitly[AsyncHandler[F]].seekAsync(reader, messageId)

  /** Returns true if the topic was terminated and this reader has reached the
    * end of the topic.
    */
