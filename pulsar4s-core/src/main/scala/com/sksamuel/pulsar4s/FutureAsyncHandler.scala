...
class FutureAsyncHandler(implicit ec: ExecutionContext)
    extends AsyncHandler[Future] {

  implicit class VoidCompletableFutureOps(
      val completableFuture: CompletableFuture[Void]
  ) {
    def toScala: Future[Unit] =
      new CompletionStageOps(completableFuture).toScala.map(_ => ())
  }

  override def failed(e: Throwable): Future[Nothing] = Future.failed(e)

  override def createProducer[T](
      builder: api.ProducerBuilder[T]
  ): Future[Producer[T]] = {
  ...
  override def createConsumer[T](
    ...
    builder
      .subscribeAsync()
      .thenApply[Consumer[T]](new DefaultConsumer(_))
      .toScala
  ...
  ): Future[Vector[ConsumerMessage[T]]] =
    consumer
      .batchReceiveAsync()
      .toScala
      .map(_.asScala.map(ConsumerMessage.fromJava).toVector)

  override def unsubscribeAsync(consumer: api.Consumer[_]): Future[Unit] =
    consumer.unsubscribeAsync().toScala
  ...
  override def close(producer: api.Producer[_]): Future[Unit] =
    producer.closeAsync().toScala

  override def close(consumer: api.Consumer[_]): Future[Unit] =
    consumer.closeAsync().toScala

  override def close(client: api.PulsarClient): Future[Unit] =
    client.closeAsync().toScala
  ...
  override def transform[A, B](f: Future[A])(fn: A => Try[B]): Future[B] =
    f.flatMap { a =>
      fn(a) match {
  ...
  override def close(reader: api.Reader[_]): Future[Unit] =
    reader.closeAsync().toScala

  override def flush(producer: api.Producer[_]): Future[Unit] =
    producer.flushAsync().toScala
  ...
  override def commitTransaction(txn: Transaction): Future[Unit] =
    txn.commit().toScala.map(_ => ())

  override def abortTransaction(txn: Transaction): Future[Unit] =
    txn.abort().toScala.map(_ => ())
