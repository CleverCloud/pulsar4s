...
sealed trait TransactionContext {
  ...
  /** Explicitly commit the transaction. Note that this action must occur
    * _after_ all other actions on the transaction.
    */
  def commit[F[_]: AsyncHandler]: F[Unit]

  /** Explicitly abort the transaction. Note that this action must occur _after_
    * all other actions on the transaction.
    */
  def abort[F[_]: AsyncHandler]: F[Unit]

  /** Get an instance of `TransactionalConsumerOps` that provides transactional
    * operations on the consumer.
    */
  final def apply[T](consumer: Consumer[T]): TransactionalConsumerOps[T] =
    consumer.tx(using this)

  /** Get an instance of `TransactionalProducerOps` that provides transactional
    * operations on the producer.
    */
  final def apply[T](producer: Producer[T]): TransactionalProducerOps[T] =
    producer.tx(using this)
}
...
object TransactionContext {
  def apply(txn: Transaction): TransactionContext = new TransactionContext {
    ...
    def commit[F[_]: AsyncHandler]: F[Unit] =
      implicitly[AsyncHandler[F]].commitTransaction(txn)

    def abort[F[_]: AsyncHandler]: F[Unit] =
      implicitly[AsyncHandler[F]].abortTransaction(txn)
  }
}
...
sealed trait TransactionBuilder {
  ...
  /** Given a `TransactionContext => F[A]`, produce an `F[A]` that runs in a new
    * transaction.
    *
  ...
  /** Given a `TransactionContext => F[Either[E, A]]`, produce an `F[Either[E,
    * A]]` that runs in a new transaction.
    *
    * If `F` fails or the result is a `Left`, abort the transaction. Otherwise
    * commit the transaction.
    */
  def runWithEither[E, A, F[_]: AsyncHandler](
...
private class TransactionBuilderImpl(
  ...
  override def runWith[T, F[_]: AsyncHandler](
      action: TransactionContext => F[T]
  ): F[T] = {
  ...
  override def runWithEither[E, A, F[_]: AsyncHandler](
      action: TransactionContext => F[Either[E, A]]
  ): F[Either[E, A]] =
...
trait PulsarClient {
  ...
  def producer[T: Schema](
      config: ProducerConfig,
      interceptors: List[ProducerInterceptor[T]] = Nil
  ): Producer[T]

  def consumer[T: Schema](
      config: ConsumerConfig,
      interceptors: List[ConsumerInterceptor[T]] = Nil
  ): Consumer[T]
...
trait PulsarAsyncClient extends PulsarClient {
  ...
  def producerAsync[T: Schema, F[_]: AsyncHandler](
      config: ProducerConfig,
      interceptors: List[ProducerInterceptor[T]] = Nil
  ): F[Producer[T]]

  def consumerAsync[T: Schema, F[_]: AsyncHandler](
      config: ConsumerConfig,
      interceptors: List[ConsumerInterceptor[T]] = Nil
  ): F[Consumer[T]]

  def readerAsync[T: Schema, F[_]: AsyncHandler](
      config: ReaderConfig
  ): F[Reader[T]]
...
class ConsumerInterceptorAdapter[T](
    interceptor: ConsumerInterceptor[T],
    schema: Schema[T]
) extends api.ConsumerInterceptor[T] {

  override def close(): Unit = interceptor.close()

  override def beforeConsume(
  ...
  override def onAcknowledge(
      consumer: api.Consumer[T],
      messageId: JMessageId,
      throwable: Throwable
  ): Unit = {
    if (throwable == null) interceptor.onAck(MessageId.fromJava(messageId))
    else interceptor.onError(MessageId.fromJava(messageId), throwable)
  }

  override def onAcknowledgeCumulative(
      consumer: api.Consumer[T],
      messageId: JMessageId,
      throwable: Throwable
  ): Unit = {
    if (throwable == null)
      interceptor.onAckCumulative(MessageId.fromJava(messageId))
    else interceptor.onErrorCumulative(MessageId.fromJava(messageId), throwable)
  }

  override def onNegativeAcksSend(
...
@deprecated
class ProducerInterceptorAdapter[T](
    interceptor: ProducerInterceptor[T],
    schema: Schema[T]
) extends api.ProducerInterceptor[T] {

  override def close(): Unit = interceptor.close()

  override def beforeSend(
      producer: api.Producer[T],
      msg: JMessage[T]
  ): JMessage[T] = {
  ...
  override def onSendAcknowledgement(
      producer: api.Producer[T],
      msg: JMessage[T],
      messageId: JMessageId,
      throwable: Throwable
    ...
    if (throwable == null)
      interceptor.onAck(
        ProducerMessage.fromJava(msg),
        MessageId.fromJava(messageId)
      )
    else
...
object PulsarClient {
  ...
  def apply(config: PulsarClientConfig): PulsarAsyncClient = {
    ...
    config.allowTlsInsecureConnection.foreach(
    ...
    config.operationTimeout
      ...
      .foreach(builder.operationTimeout(_, TimeUnit.SECONDS))
    config.keepAliveInterval
      .map(_.toSeconds.toInt)
      .foreach(builder.keepAliveInterval(_, TimeUnit.SECONDS))
    config.statsInterval
      .map(_.toMillis)
      .foreach(builder.statsInterval(_, TimeUnit.MILLISECONDS))
  ...
  def apply(serviceUrl: String): PulsarAsyncClient = apply(
    PulsarClientConfig(serviceUrl)
  )
}

class DefaultPulsarClient(client: org.apache.pulsar.client.api.PulsarClient)
    extends PulsarAsyncClient
    with Logging {

  override def close(): Unit = client.close()

  private def producerBuilder[T](
      config: ProducerConfig,
      interceptors: List[ProducerInterceptor[T]]
  )(implicit schema: Schema[T]): ProducerBuilder[T] = {
    ...
    config.batchingMaxPublishDelay
      .map(_.toMillis)
      .foreach(builder.batchingMaxPublishDelay(_, TimeUnit.MILLISECONDS))
    ...
    config.maxPendingMessagesAcrossPartitions.foreach(
      builder.maxPendingMessagesAcrossPartitions
    )
    config.roundRobinRouterBatchingPartitionSwitchFrequency.foreach(
    ...
    config.sendTimeout
      ...
      .foreach(builder.sendTimeout(_, TimeUnit.MILLISECONDS))
    config.batcherBuilder.foreach(builder.batcherBuilder)
    if (interceptors.nonEmpty)
      builder.intercept(
        interceptors.map(new ProducerInterceptorAdapter(_, schema)): _*
      )
    if (config.additionalProperties.nonEmpty)
      builder.loadConf(config.additionalProperties.asJava)
    builder
  ...
  private def consumerBuilder[T](
      config: ConsumerConfig,
      interceptors: List[ConsumerInterceptor[T]] = Nil
  )(implicit schema: Schema[T]): ConsumerBuilder[T] = {
    ...
    config.cryptoFailureAction.foreach(builder.cryptoFailureAction)
    config.cryptoKeyReader.foreach(builder.cryptoKeyReader)
    config.maxTotalReceiverQueueSizeAcrossPartitions.foreach(
      builder.maxTotalReceiverQueueSizeAcrossPartitions
    )
    config.negativeAckRedeliveryDelay.foreach { d =>
      builder.negativeAckRedeliveryDelay(d._1, d._2)
    }
    config.autoUpdatePartitions.foreach(builder.autoUpdatePartitions)
    config.patternAutoDiscoveryPeriod.foreach { d =>
      builder.patternAutoDiscoveryPeriod(d.length.toInt, d.unit)
    }
    ...
    config.subscriptionInitialPosition.foreach(
    ...
    config.ackTimeoutTickTime.foreach { tt =>
      builder.ackTimeoutTickTime(tt._1, tt._2)
    }
    config.enableBatchIndexAcknowledgement.foreach { enabled =>
      builder.enableBatchIndexAcknowledgment(enabled)
    }
    config.deadLetterPolicy.foreach(builder.deadLetterPolicy)
    config.batchReceivePolicy.foreach(builder.batchReceivePolicy)
    config.acknowledgmentGroupTime.foreach { gt =>
      builder.acknowledgmentGroupTime(gt._1, gt._2)
    }
    ...
    if (interceptors.nonEmpty)
      builder.intercept(
        interceptors.map(new ConsumerInterceptorAdapter(_, schema)): _*
      )
    if (config.additionalProperties.nonEmpty)
      builder.loadConf(config.additionalProperties.asJava)
  ...
  private def readerBuilder[T](
      config: ReaderConfig
  )(implicit schema: Schema[T]): ReaderBuilder[T] = {
    ...
    config.startMessage match {
      case Message(messageId) =>
        builder.startMessageId(MessageId.toJava(messageId))
      case RollBack(rollbackDuration, timeunit) =>
        builder.startMessageFromRollbackDuration(rollbackDuration, timeunit)
    }
  ...
  override def producerAsync[T: Schema, F[_]: AsyncHandler](
      config: ProducerConfig,
      interceptors: List[ProducerInterceptor[T]] = Nil
  ): F[Producer[T]] =
    implicitly[AsyncHandler[F]].createProducer(
      producerBuilder(config, interceptors)
    )

  override def consumerAsync[T: Schema, F[_]: AsyncHandler](
      config: ConsumerConfig,
      interceptors: List[ConsumerInterceptor[T]]
  ): F[Consumer[T]] =
    implicitly[AsyncHandler[F]].createConsumer(
      consumerBuilder(config, interceptors)
    )

  override def readerAsync[T: Schema, F[_]: AsyncHandler](
      config: ReaderConfig
  ): F[Reader[T]] = {
    implicitly[AsyncHandler[F]].createReader(readerBuilder(config))
  }

  override def transaction: TransactionBuilder = new TransactionBuilderImpl(
    client
  )
}
