...
trait TransactionalConsumerOps[T] {
  final def acknowledgeAsync[F[_]: AsyncHandler](
  ...
  final def acknowledgeCumulativeAsync[F[_]: AsyncHandler](
      message: ConsumerMessage[T]
  ): F[Unit] =
    acknowledgeCumulativeAsync(message.messageId)

  def acknowledgeCumulativeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit]
}
...
trait Consumer[T] extends Closeable with TransactionalConsumerOps[T] {
  ...
  /** Receive a single message waiting up to the given duration if necessary. If
    * no message is received within the duration then None is returned.
    */
  ...
  /** Return true if the topic was terminated and this consumer has already
    ...
    * with the last message published by producers, rather the topic needs to be
  ...
  def acknowledge(message: ConsumerMessage[T]): Unit = acknowledge(
    message.messageId
  )
  def acknowledge(messageId: MessageId): Unit

  def negativeAcknowledge(message: ConsumerMessage[T]): Unit =
  ...
  final def negativeAcknowledgeAsync[F[_]: AsyncHandler](
      message: ConsumerMessage[T]
  ): F[Unit] =
    negativeAcknowledgeAsync(message.messageId)

  def negativeAcknowledgeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit]
  ...
  def reconsumeLaterAsync[F[_]: AsyncHandler](
      message: ConsumerMessage[T],
      delayTime: Long,
      unit: TimeUnit
  ): F[Unit]
...
class DefaultConsumer[T](consumer: JConsumer[T])
    extends Consumer[T]
    with Logging {
  ...
  ): Try[Option[ConsumerMessage[T]]] = Try {
    logger.trace(
      s"About to block for duration $duration or until a message is received.."
    )
    val msg = consumer.receive(duration.toMillis.toInt, TimeUnit.MILLISECONDS)
    Option(msg).map(ConsumerMessage.fromJava)
  }

  override def receiveAsync[F[_]: AsyncHandler]: F[ConsumerMessage[T]] =
    implicitly[AsyncHandler[F]].receive(consumer)

  override def receiveBatchAsync[F[_]: AsyncHandler]
  ...
  override def acknowledgeCumulative(message: ConsumerMessage[T]): Unit =
    consumer.acknowledgeCumulative(message.messageId)
  override def acknowledgeCumulative(messageId: MessageId): Unit =
    consumer.acknowledgeCumulative(messageId)

  override def acknowledgeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit] =
    implicitly[AsyncHandler[F]].acknowledgeAsync(consumer, messageId)

  override def acknowledgeCumulativeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit] =
    implicitly[AsyncHandler[F]].acknowledgeCumulativeAsync(consumer, messageId)

  override def negativeAcknowledge(messageId: MessageId): Unit =
    consumer.negativeAcknowledge(messageId)
  override def negativeAcknowledgeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit] =
    implicitly[AsyncHandler[F]].negativeAcknowledgeAsync(consumer, messageId)

  override def reconsumeLater(
      message: ConsumerMessage[T],
      delayTime: Long,
      unit: TimeUnit
  ): Unit =
    message match {
      case consumerMessage: ConsumerMessageWithValueTry[T] =>
        consumer.reconsumeLater(
          consumerMessage.getBaseMessage(),
          delayTime,
          unit
        )
      case _ =>
  ...
  override def reconsumeLaterAsync[F[_]: AsyncHandler](
      message: ConsumerMessage[T],
      delayTime: Long,
      unit: TimeUnit
  ): F[Unit] =
    implicitly[AsyncHandler[F]].reconsumeLaterAsync(
      consumer,
      message,
      delayTime,
      unit
  ...
  override def subscription: Subscription = Subscription(
  ...
  override def getLastMessageIdAsync[F[_]: AsyncHandler]: F[MessageId] =
    implicitly[AsyncHandler[F]].getLastMessageId(consumer)
  ...
  override def closeAsync[F[_]: AsyncHandler]: F[Unit] =
    implicitly[AsyncHandler[F]].close(consumer)

  override def unsubscribe(): Unit = consumer.unsubscribe()
  override def unsubscribeAsync[F[_]: AsyncHandler]: F[Unit] =
    implicitly[AsyncHandler[F]].unsubscribeAsync(consumer)

  override def tx(implicit
      ctx: TransactionContext
  ): TransactionalConsumerOps[T] =
    new DefaultTransactionalConsumer[T](consumer, ctx.transaction)
}

class DefaultTransactionalConsumer[T](
    consumer: JConsumer[T],
    transaction: Transaction
) extends TransactionalConsumerOps[T] {
  override def acknowledgeAsync[F[_]: AsyncHandler](
      messageId: MessageId
  ): F[Unit] =
    implicitly[AsyncHandler[F]].acknowledgeAsync(
      consumer,
      messageId,
      transaction
    )
  ...
  override def acknowledgeCumulativeAsync[F[_]: AsyncHandler](
    ...
    implicitly[AsyncHandler[F]].acknowledgeCumulativeAsync(
      consumer,
      messageId,
      transaction
    )
