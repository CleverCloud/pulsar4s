...
trait TransactionalProducerOps[T] {
  ...
  /** Asynchronously sends a message of type T, returning an effect which will
    * be completed with the [[MessageId]] once the message is acknowledged by
    * the Pulsar broker.
    *
    * This method can be used when you have no need to set the other properties
    * of a message, such as the event time, key, headers and so on. The producer
    * will generate an appropriate Pulsar [[ProducerMessage]] with this t set as
    * the value.
    */
  def sendAsync[F[_]: AsyncHandler](t: T): F[MessageId]

  /** Asynchronously sends a [[ProducerMessage]] of type T, returning an effect
    * which will be completed with the [[MessageId]] once the message is
    * acknowledged by the Pulsar broker.
    *
    * This method can be used when you want to specify properties on a message
    * such as the event time, key and so on.
    */
...
trait Producer[T]
    extends Closeable
    with Logging
    with TransactionalProducerOps[T] {

  /** Returns the [[ProducerName]] which could have been specified by the client
  ...
  /** Sends a message of type T. The message sent will have no key.
    ...
    * the value.
    *
    * This call will block until it is successfully acknowledged by the Pulsar
    * broker.
    */
  def send(t: T): Try[MessageId]

  /** Sends a [[ProducerMessage]] of type T. This method can be used when you
    * want to specify properties on a message such as the event time, key and so
    * on.
    *
    * This call will block until it is successfully acknowledged by the Pulsar
    * broker.
  ...
  /** Get the last sequence id that was published by this producer.
    *
    * This represented either the automatically assigned or custom sequence id
    ...
    * the last message that was published in the previous producer session, or
    * -1 if there no message was ever published.
    */
  ...
  /** Close the [[Producer]] and releases resources allocated.
    ...
    * pending write request are persisted. In case of errors, pending writes
    * will not be retried.
    */
  def close(): Unit

  /** Close the [[Producer]], releases resources allocated, and returns an
    * effect that is completed when the close operation has completed.
    *
    * No more writes will be accepted from this producer. Waits until all
    * pending write request are persisted. In case of errors, pending writes
    * will not be retried.
...
class DefaultProducer[T](producer: JProducer[T]) extends Producer[T] {
  ...
  override final def sendAsync[F[_]: AsyncHandler](t: T): F[MessageId] =
    sendAsync(ProducerMessage(t))

  override def send(msg: ProducerMessage[T]): Try[MessageId] = Try(
    buildMessage(msg).send()
  )
  override def sendAsync[F[_]: AsyncHandler](
      msg: ProducerMessage[T]
  ): F[MessageId] = AsyncHandler[F].send(buildMessage(msg))

  override def lastSequenceId: SequenceId = SequenceId(
  ...
  override def flushAsync[F[_]: AsyncHandler]: F[Unit] =
    AsyncHandler[F].flush(producer)
  ...
  override def closeAsync[F[_]: AsyncHandler]: F[Unit] =
    AsyncHandler[F].close(producer)

  override def tx(implicit
      ctx: TransactionContext
  ): TransactionalProducerOps[T] =
...
class DefaultTransactionalProducer[T](
    producer: JProducer[T],
    transaction: Transaction
) extends DefaultProducer[T](producer) {
  override protected def buildMessage(
      msg: ProducerMessage[T]
  ): TypedMessageBuilder[T] =
    new ProducerMessageBuilder[T](producer, Some(transaction)).build(msg)
}

class ProducerMessageBuilder[T](
    producer: JProducer[T],
  ...
  def build(msg: ProducerMessage[T]): TypedMessageBuilder[T] = {
    ...
    val builder = transaction
      .fold(producer.newMessage())(producer.newMessage)
      .value(msg.value)
