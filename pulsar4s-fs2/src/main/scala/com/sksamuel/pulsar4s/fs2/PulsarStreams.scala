...
object PulsarStreams {
  ...
  private final class DelegateCommittableMessage[F[_]: AsyncHandler, T](
    ...
    override def map[Y](f: T => Y): CommittableMessage[F, Y] =
      new DelegateCommittableMessage(message, f(payload))
  }
  ...
  private final case class ConsumerCommittableMessage[F[_]: AsyncHandler, T](
    ...
    override def nack: F[Unit] =
      consumer.negativeAcknowledgeAsync(message.messageId)
  ...
  def batch[F[_]: MonadCancelThrow: AsyncHandler, T](
    ...
    Stream
      .resource(Resource.make(consumer)(_.closeAsync))
      .flatMap { consumer =>
        Stream
          .repeatEval(consumer.receiveBatchAsync[F])
          .flatMap(Stream.emits(_))
          .mapChunks(
            _.map(message => ConsumerCommittableMessage(message, consumer))
          )
      }
  ...
  def reader[F[_]: MonadCancelThrow: AsyncHandler, T](
    ...
    Stream
      .resource(Resource.make(reader)(_.closeAsync))
      .flatMap { reader =>
        Stream
  ...
  def sink[F[_]: MonadCancelThrow: AsyncHandler, T](
    ...
    Stream
      .resource(Resource.make(producer)(_.closeAsync))
      .flatMap { producer =>
    ...
    Stream
      ...
      .flatMap { producer =>
        messages.evalMap { message =>
          MonadCancel[F, Throwable].guaranteeCase(
            producer.sendAsync(message.data)
          ) { out =>
