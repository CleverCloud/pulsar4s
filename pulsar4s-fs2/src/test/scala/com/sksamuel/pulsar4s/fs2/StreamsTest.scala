...
class StreamsTest extends AnyFunSuite with Matchers with BeforeAndAfterAll {
  ...
  private def publishMessages[F[_]: Sync: AsyncHandler](
      client: PulsarAsyncClient,
      t: Topic,
      messages: List[String]
  ): F[Unit] =
    for {
      producer <- client.producerAsync[String, F](
        ProducerConfig(
          topic = t
        )
      )
  ...
  test("able to read from subscription [batch]") {
    ...
    (for {
      ...
      batch <- PulsarStreams
        .batch[IO, String](
          client.consumerAsync[String, IO](
            ConsumerConfig(
  ...
  test("able to connect with sink") {
    ...
    (for {
      ...
      _ <- PulsarStreams
        ...
        .through(
          PulsarStreams.committableSink(
            client.producerAsync[String, IO](ProducerConfig(topic2))
          )
        )
        .compile
        .drain
