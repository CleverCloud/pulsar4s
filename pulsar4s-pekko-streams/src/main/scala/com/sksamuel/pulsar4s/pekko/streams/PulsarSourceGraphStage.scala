...
trait Control {
  ...
  /** Complete the source but leave the Pulsar consumer open to receive any
    * unacked messages.
    *
    * @return
  ...
  /** Shut down the Pulsar consumer, and shut down the `Source` if it is not
    * already shut down.
    *
    * @return
    *   a future completed with `Done` when the source is shut down and the
    *   consumer is closed.
    */
  def shutdown()(implicit ec: ExecutionContext): Future[Done]

  /** Stop producing messages from the `Source`, wait for stream completion and
    * shut down the consumer `Source` so that all consumed messages reach the
    * end of the stream. Failures in stream completion will be propagated.
    */
  def drainAndShutdown[S](
      streamCompletion: Future[S]
...
class PulsarSourceGraphStage[T](
    create: () => Consumer[T],
