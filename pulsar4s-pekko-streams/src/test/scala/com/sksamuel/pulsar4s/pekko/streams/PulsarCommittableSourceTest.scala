...
class PulsarCommittableSourceTest extends AnyFunSuite with Matchers {
  ...
  private val specialStringSchema: Schema[String] = new Schema[String] {
    ...
    override def encode(message: String) =
      ("message:" + message).getBytes("UTF-8")
    override def decode(data: Array[Byte]) =
  ...
  test(
    ...
    val f = committableSource(createFn, Some(MessageId.earliest), 2.seconds)
      .take(1)
      .runWith(Sink.seq[CommittableMessage[String]])
    val msgs = Await.result(f, 15.seconds)
  ...
  test("shutdown of failed source does not cause an exception") {
    ...
    val (control, doneFt) =
      committableSource(createFn, Some(MessageId.earliest), 30.seconds)
