...
class PulsarMultiSinkTest extends AnyFunSuite with Matchers {
  ...
  test(
    "pulsar multi sink should write single-topic messages to pulsar cluster"
  ) {
    val topic =
      Topic("persistent://sample/standalone/ns1/sinktest_" + UUID.randomUUID)

    val producerFn = (topic: Topic) => client.producer(ProducerConfig(topic))
    val f = Source
      .fromIterator(() => List("a", "b", "c", "d").iterator)
    ...
    Iterator
      .continually(consumer.receive(30.seconds).get)
      .take(4)
      .toList
      .flatten
      .size shouldBe 4
  ...
  test(
    "pulsar multi sink should write multi-topic messages to pulsar cluster"
  ) {
    val topic1 =
      Topic("persistent://sample/standalone/ns1/sinktest_" + UUID.randomUUID)
    val topic2 =
    ...
    val f = Source
      .fromIterator(() =>
        (List("a", "b", "c", "d").map(x => topic1 -> x) ++ List(
          "e",
          "f",
          "g",
          "h"
        ).map(x => topic2 -> x)).iterator
      )
    ...
    Iterator
      .continually(consumer1.receive(30.seconds).get)
      .take(4)
      .toList
      .flatten
      .size shouldBe 4
    ...
    Iterator
      .continually(consumer2.receive(30.seconds).get)
      .take(4)
      .toList
      .flatten
      .size shouldBe 4
