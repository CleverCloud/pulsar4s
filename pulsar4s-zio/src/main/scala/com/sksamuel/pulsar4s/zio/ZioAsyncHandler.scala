...
class ZioAsyncHandler extends AsyncHandler[Task] {
  ...
  override def createProducer[T](
    ...
    fromFuture(ZIO.attempt(builder.createAsync())) flatMap (p =>
      ZIO.attempt(new DefaultProducer(p))
    )

  override def createConsumer[T](
      builder: ConsumerBuilder[T]
  ): Task[pulsar4s.Consumer[T]] =
    fromFuture(ZIO.attempt(builder.subscribeAsync())) flatMap (p =>
      ZIO.attempt(new DefaultConsumer(p))
    )
  ...
  override def createReader[T](
    ...
    fromFuture(ZIO.attempt(builder.createAsync())) flatMap (p =>
      ZIO.attempt(new DefaultReader(p))
    )
  ...
  override def receive[T](consumer: api.Consumer[T]): Task[ConsumerMessage[T]] =
    fromFuture(ZIO.attempt(consumer.receiveAsync())) flatMap (v =>
      ZIO.attempt(ConsumerMessage.fromJava(v))
    )

  override def receiveBatch[T](
      consumer: Consumer[T]
  ): Task[Vector[ConsumerMessage[T]]] =
    fromFuture(ZIO.attempt(consumer.batchReceiveAsync())) flatMap (v =>
      ZIO
  ...
  override def getLastMessageId[T](consumer: api.Consumer[T]): Task[MessageId] =
    fromFuture(ZIO.attempt(consumer.getLastMessageIdAsync)) flatMap (v =>
      ZIO.attempt(MessageId.fromJava(v))
    )
  ...
  override def nextAsync[T](reader: Reader[T]): Task[ConsumerMessage[T]] =
    fromFuture(ZIO.attempt(reader.readNextAsync())) flatMap (v =>
      ZIO.attempt(ConsumerMessage.fromJava(v))
    )
  ...
  override def acknowledgeCumulativeAsync[T](
    ...
    fromFuture(
      ZIO.attempt(consumer.acknowledgeCumulativeAsync(messageId, txn))
    ).unit
  ...
  override def withTransaction[E, A](
    ...
    ZIO.acquireReleaseExitWith[Any, Throwable, TransactionContext](
      startTransaction(builder)
    )((txn: TransactionContext, e: Exit[Throwable, Either[E, A]]) =>
      (txn, e) match {
  ...
  override def commitTransaction(txn: Transaction): Task[Unit] = fromFuture(
    ZIO.attempt(txn.commit())
  ).unit

  override def abortTransaction(txn: Transaction): Task[Unit] = fromFuture(
    ZIO.attempt(txn.abort())
  ).unit
  ...
  override def reconsumeLaterAsync[T](
    ...
    ZIO
      ...
      .map(pulsarMessage =>
        consumer.reconsumeLater(pulsarMessage, delayTime, unit)
      )
}
