...
class ZioAsyncHandlerTest
  ...
  private val topic = Topic(
    "persistent://sample/standalone/ns1/zio_" + UUID.randomUUID()
  )
  ...
  test("async producer should use zio") {
    ...
    val r = Unsafe.unsafe(implicit unsafe =>
      zio.Runtime.default.unsafe
        .run(t.either.map(_.toOption.get))
  ...
  test("async consumer getMessageById should use zio") {
    ...
    val value = Unsafe.unsafe(implicit unsafe =>
      zio.Runtime.default.unsafe
        .run(receive.either.map(_.toOption.get))
        .getOrThrowFiberFailure()
    )
  ...
  test("producer and consumer can execute a transaction using zio") {
    ...
    consumer.seekEarliest()
    val msgIdIO =
      client.transaction.withTimeout(1.second).runWith[MessageId, Task] {
        implicit txn =>
          for {
    ...
    Unsafe.unsafe(implicit unsafe =>
      zio.Runtime.default.unsafe.run(msgIdIO).getOrThrowFiberFailure()
    )
    consumer.close()
    producer.close()
  }
