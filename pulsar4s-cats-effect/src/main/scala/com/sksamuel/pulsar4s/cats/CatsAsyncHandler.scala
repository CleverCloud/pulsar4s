...
trait CatsAsyncHandlerLowPriority {
  ...
  object CompletableFutureConverters extends Logging {
    ...
    implicit class CompletableOps[F[_]: Async, T](
        f: => F[CompletableFuture[T]]
    ) {
      def liftF: F[T] = {
        f.flatMap { f =>
          Async[F].defer {
  ...
  implicit def asyncHandlerForCatsEffectAsync[F[_]: Async]: AsyncHandler[F] =
    new AsyncHandler[F] with Logging {
      ...
      override def close(producer: api.Producer[_]): F[Unit] = Async[F]
        .delay {
          producer.closeAsync()
        }
        .liftF
      ...
      override def close(consumer: api.Consumer[_]): F[Unit] = Async[F]
        .delay {
          consumer.closeAsync()
        }
        .liftF
      ...
      override def acknowledgeCumulativeAsync[T](
        ...
        Async[F]
          .delay(consumer.acknowledgeCumulativeAsync(messageId, txn))
          .liftF
          .void
      ...
      override def close(reader: api.Reader[_]): F[Unit] = Async[F]
        .delay {
          reader.closeAsync()
        }
        .liftF
      ...
      override def flush(producer: api.Producer[_]): F[Unit] = Async[F]
        .delay {
          producer.flushAsync()
        }
        .liftF
      ...
      override def close(client: api.PulsarClient): F[Unit] = Async[F]
        .delay {
          client.closeAsync()
        }
        .liftF
      ...
      def commitTransaction(txn: Transaction): F[Unit] =
        Async[F].delay(txn.commit()).liftF.map(_ => ())

      def abortTransaction(txn: Transaction): F[Unit] =
        Async[F].delay(txn.abort()).liftF.map(_ => ())
      ...
      override def reconsumeLaterAsync[T](
        ...
        Async[F]
          ...
          .flatMap(pulsarMessage =>
            Async[F]
              .delay(consumer.reconsumeLater(pulsarMessage, delayTime, unit))
          )
      }
    }
