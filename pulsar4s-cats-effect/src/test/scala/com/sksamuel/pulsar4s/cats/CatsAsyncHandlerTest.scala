...
class CatsAsyncHandlerTest
  ...
  private val topic = Topic(
    "persistent://sample/standalone/ns1/cats_" + UUID.randomUUID()
  )
  ...
  test(
    "async producer should be able to use cats IO with the standard import"
  ) {
  ...
  test(
    "async consumer should be able to use cats IO with the standard import"
  ) {
    import CatsAsyncHandler._
  ...
  test(
    "async consumer getMessageById should be able to use cats IO with the standard import"
  ) {
    import CatsAsyncHandler._
    val consumer = client.consumer(
      ConsumerConfig(
  ...
  def syncResources[F[_]: Async: AsyncHandler](
      c: PulsarClient,
      t: Topic,
      subscription: Subscription
  ): Resource[F, (Producer[String], Consumer[String])] = {
    ...
    val consumer: Resource[F, Consumer[String]] = Resource.make(Sync[F].delay {
      c.consumer(
        ConsumerConfig(topics = Seq(t), subscriptionName = subscription)
      )
    }) {
      _.closeAsync
    }
  ...
  def asyncResources[F[_]: Async: AsyncHandler](
      c: PulsarAsyncClient,
      t: Topic,
      subscription: Subscription
  ): Resource[F, (Producer[String], Consumer[String])] = {
    for {
      producer <- Resource.make(c.producerAsync(ProducerConfig(t)))(
        _.closeAsync
      )
  ...
  def asyncProgram[F[_]: Async: AsyncHandler](
      producer: Producer[String],
      consumer: Consumer[String],
      message: String
  ): F[ConsumerMessage[String]] = for {
    _ <- producer.sendAsync(message)
    result <- consumer.receiveAsync
  } yield result

  def asyncBatchProgram[F[_]: Async: AsyncHandler](
      producer: Producer[String],
      consumer: Consumer[String],
      message: String
  ): F[Vector[ConsumerMessage[String]]] = for {
    _ <- (0 to 5).toList
      .map(id => producer.sendAsync[F](s"${message}_$id"))
      .sequence
  ...
  test("async consumer/producer/reader should be lazy") {
    import CatsAsyncHandler._
    val topic =
      Topic("persistent://sample/standalone/ns1/" + UUID.randomUUID().toString)

    val program = Resource
      .make(client.producerAsync(ProducerConfig(topic)))(_.closeAsync)
      .use {
        _.sendAsync("test_message")
      }
    val checkAvailable = Resource
      .make(
        client.readerAsync(
          ReaderConfig(topic, startMessage = Message(MessageId.earliest))
        )
      )(_.closeAsync)
      .use {
  ...
  def processResource[F[_]: Async: AsyncHandler, X](
    ...
    pulsarResource.use { case (producer, consumer) =>
  ...
  test("async batch methods should return batch instead of single message") {
    ...
    processResource[IO, Vector[ConsumerMessage[String]]](
      syncResources[IO](client, topic, subscription)
    ) { (producer, consumer) =>
      asyncBatchProgram[IO](producer, consumer, msg)
    }.map(_.length).unsafeRunSync() should be > 1

    processResource[IO, Vector[ConsumerMessage[String]]](
      asyncResources[IO](client, topic, subscription)
    ) { (producer, consumer) =>
      asyncBatchProgram[IO](producer, consumer, msg)
    }.map(_.length).unsafeRunSync() should be > 1
  }
  ...
  test(
    "async client methods should work with any monad which implements Async - IO"
  ) {
    import CatsAsyncHandler._
    ...
    processResource[IO, ConsumerMessage[String]](
      syncResources[IO](client, topic, subscription)
    ) { (producer, consumer) =>
  ...
  test(
    "async client methods should work with any monad which implements Async - StateT[IO, Int, ?]"
  ) {
    ...
    type F[T] =
      WriterT[
        ...
        Int,
        T
      ] // this would be `StateT[IO, Int, ?]` with kind projector
    ...
    processResource[IO, (Int, ConsumerMessage[String])](
      syncResources[IO](client, topic, subscription)
    ) { (producer, consumer) =>
      asyncProgram[F](producer, consumer, msg).tell(rnd).run
    }.map { case (a, msg) => (a, msg.value) }
      .unsafeRunSync() shouldBe (rnd, msg)
...
//  }
  ...
  test(
    "async client methods should work with any monad which implements Async - ZIO"
  ) {
    import CatsAsyncHandler.asyncHandlerForCatsEffectAsync
    ...
    val sync = zio.Runtime.default.run {
      processResource[zio.Task, ConsumerMessage[String]](
        syncResources[zio.Task](client, topic, subscription)
      ) { (producer, consumer) =>
    ...
    val async = zio.Runtime.default.run {
      processResource[zio.Task, ConsumerMessage[String]](
        asyncResources[zio.Task](client, topic, subscription)
      ) { (producer, consumer) =>
        asyncProgram[zio.Task](producer, consumer, msg)
      }.map(_.value)
    }
    ...
    Unsafe.unsafe(implicit unsafe =>
      zio.Runtime.default.unsafe.run(sync).getOrThrowFiberFailure()
    ) should be(msg)
    Unsafe.unsafe(implicit unsafe =>
      zio.Runtime.default.unsafe.run(async).getOrThrowFiberFailure()
    ) should be(msg)
  }
}
